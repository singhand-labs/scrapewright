...
5. **Sticky tab activation** (RC20 origin, RC56 sticky rewrite, `lib/tab-activation.js`, default on): addresses Chrome's hard architectural rule — compositor frames are produced *only* for the active tab in the focused window, so both IO callbacks AND CDP Input.dispatchMouseEvent require activation. `TabActivation.requestActivation(tabId)` activates the scrape tab and **KEEPS it active** (RC20's per-op restore caused activate/restore churn on back-to-back ops); the next op simply re-activates if the user switched away. The user's last-clicked tab is tracked via `chrome.tabs.onActivated` with a suppression Set (tabIds of our own pending `tabs.update`, cleared on the matching event or a 1000ms safety timer); when a tab closes while active (scrape-tab auto-close), focus lands on the last-clicked tab via `chrome.tabs.update`, focusing its window too if different. State persists to `chrome.storage.session` (the MV3 SW can suspend during page-context LLM calls). Listeners register at SW top level (`background.js` `initTabActivationListeners()`). Wrapped ops: `domScrollToBottom`, `domHover`, `hoverDismiss` — `content-script.js withTabActivation(label, fn)` sends `TAB_ACTIVATION_REQUEST` only (sticky: no release message). Universality: pure infrastructure, no site-specific logic, no prompt changes.
