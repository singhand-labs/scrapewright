...
5. **Brief tab activation** (RC20, `lib/tab-activation.js`, default on): wraps input-required DOM ops so the scrape tab is briefly the active tab in the focused window during the op. This is the **only** layer that addresses Chrome's hard architectural rule — compositor frames are produced *only* for the active tab in the focused window, so both IO callbacks AND CDP Input.dispatchMouseEvent require activation. The four prior layers were all necessary-but-insufficient: they address throttle/lifecycle/input-trust but not frame production. Relay chain: `content-script.js withTabActivation(fn)` → `chrome.runtime.sendMessage(TAB_ACTIVATION_REQUEST)` → `background.js` → `TabActivation.requestActivation(tabId)` (which calls `chrome.tabs.update({active:true})` and saves the restore target). After `fn` resolves, `TAB_ACTIVATION_RELEASE` calls `TabActivation.releaseActivation(tabId)` which restores the previous active tab unless the user manually changed tabs mid-op. Safety: no-op if scrape tab is already active; refuses cross-window activation (scrape tab in W1, user focus in W2); restore-skip if user switched tabs during op (don't fight the user). Today wraps `domScrollToBottom` only; other input ops can adopt it incrementally. Universality: pure infrastructure, no site-specific logic, no prompt changes.
