...
| 2. Enhanced Scraping Mode | `lib/renderer-activation.js` (opt-in via options-page toggle, backed by `chrome.storage.local` flag `enhancedModeEnabled`) | Gates availability of the trusted-wheel fallback (layer 4). Previously also ran `Page.setWebLifecycleState` via `chrome.debugger` (RC18 Plan A); removed in RC20 as redundant — brief tab activation (layer 5) makes lifecycle naturally ACTIVE during the input window. | Compositor frame production. Detection-risk minimized: when the flag is on, only `Input.*` CDP commands are issued, never `Runtime.*`/`Network.*`/`DOM.*`. |
...
| 4. Trusted-wheel fallback (RC19) | `lib/renderer-activation.js:dispatchTrustedWheelScroll` + `trustedWheelFallback` option of `scroll-ops.js:scrollToBottomIncremental` (shares the `enhancedModeEnabled` flag with layer 2) | When programmatic `scrollBy` stalls (scrollHeight stops growing), scroll-ops calls `dispatchTrustedWheelScroll`: transient `chrome.debugger` attach, send `Input.dispatchMouseEvent({type:'mouseMoved'})` + `Input.dispatchMouseEvent({type:'mouseWheel', deltaY})`. CDP input runs through Chrome's real input pipeline, so the resulting wheel event has `event.isTrusted=true` — the **only** programmatic way to produce a trusted wheel event. Relay chain: `content-script.js` → `chrome.runtime.sendMessage(TRUSTED_WHEEL_SCROLL_REQUEST)` → `background.js` → `RendererActivation.dispatchTrustedWheelScroll(sender.tab.id)`. Detection-risk: only `Input.*` CDP commands added (no `Runtime.*`/`Network.*`/`DOM.*`). `DEFAULT_MAX_TRUSTED_WHEEL_ATTEMPTS=3`. Universality: no site-specific logic in prompts or DSL — the LLM keeps writing `$scrollToBottom`, infrastructure dispatches trusted wheel events transparently when programmatic scroll stalls. | Requires Enhanced Scraping Mode on. Only fires on stall — sites that respond to programmatic scroll are unaffected. |
| 5. Brief tab activation (RC20) | `lib/tab-activation.js` (default on) | Wraps input-required DOM ops (`$scrollToBottom`, future: `$click`) so the scrape tab is briefly the active tab in the focused window during the op, then restores the user's previous active tab. The **only** layer that addresses Chrome's hard architectural rule — compositor frames are produced *only* for the active tab in the focused window, so both IO callbacks AND CDP Input.dispatchMouseEvent require activation. Relay chain: `content-script.js withTabActivation(fn)` → `chrome.runtime.sendMessage(TAB_ACTIVATION_REQUEST)` → `background.js` → `TabActivation.requestActivation(tabId)` (calls `chrome.tabs.update({active:true})`, saves restore target); on `fn` resolve, `TAB_ACTIVATION_RELEASE` restores previous tab unless user switched mid-op. Safety: no-op if already active; refuses cross-window activation; restore-skip if user switched tabs (don't fight the user). | None — this is the architectural conclusion. The four prior layers were all necessary-but-insufficient because they address throttle/lifecycle/input-trust, not frame production. |
...
The popup-window path (`chrome.windows.create({type:'popup', focused:false})` from RC12/RC17) was removed in RC20 — brief tab activation (layer 5) makes background tabs work without the focus-stealing cost of popups. `closeScrapeTab(tab)` is now a thin idempotent wrapper over `chrome.tabs.remove`.
