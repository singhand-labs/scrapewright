...
Scrapewright 将大语言模型（LLM）与浏览器自动化深度结合：用户只需用自然语言描述采集需求，系统即可自动分析目标网页、生成采集脚本、执行并返回结构化数据。项目以 Chrome 扩展（Manifest V3）+ Node.js Native Messaging Host 的双层架构实现，对外暴露标准 HTTP API，可便捷集成到任意后端系统、数据管道或自动化工作流中。
...
传统的网页采集方案（Scrapy、Puppeteer、BeautifulSoup 等）存在几个核心痛点：

1. **开发成本高** — 每个目标网站都需要手写选择器、处理翻页、应对反爬，维护成本随网站变化不断累积
2. **动态页面难以处理** — SPA、iframe 嵌套、JavaScript 动态渲染的内容，传统基于 HTTP 请求的方式难以覆盖
3. **难以复用** — 针对每个网站的采集脚本都是定制的，无法快速迁移到结构类似的页面
4. **缺乏统一接口** — 不同采集任务之间没有标准化的输入输出格式，难以编排和扩展

Scrapewright 的应对之道：

- **AI 驱动**：用户只需描述"要什么"，LLM 自动分析页面结构、生成采集脚本、遇到错误自动修复
- **真实浏览器环境**：基于 Chrome 扩展，在完整的浏览器中执行，天然支持 JavaScript 渲染、iframe 穿透与动态加载
- **标准化 API**：所有采集服务通过统一的 HTTP API 调用，输入输出均有 JSON Schema 约束
- **可视化向导**：7 步向导流程，从描述需求到测试部署全程可视化，非技术人员也能使用

> 更详细的设计理念与产品定位，见 [`docs/technical-introduction.md`](./docs/technical-introduction.md)。
...
在 Options 页点击 **"+ New Service"**，进入 AI 向导（7 步流程）：

| 步骤 | 说明 |
|------|------|
| **Step 1: Target URL** | 输入目标网站 URL（按 Enter 可直接下一步） |
| **Step 2: Describe Needs** | 自然语言描述采集需求，按 **Research** 按钮（或 Ctrl+Enter），AI 分析页面并生成脚本 |
| **Step 3: Annotate Elements** | 如 AI 需要帮助，可视化标注页面元素；标注完成后 AI 会根据标注自动优化脚本 |
| **Step 4: Service Name & Script** | 设定服务名称，查看和**编辑** AI 生成的脚本 |
| **Step 5: I/O Schema & Test Input** | 确认输入输出参数格式（JSON Schema），编辑测试输入数据 |
| **Step 6: Execute Test** | 实时查看执行日志（打开页面 → 加载 → 执行 → 完成/失败） |
| **Step 7: Results** | 查看测试结果。失败时可 **Auto-Fix**（AI 自动修复）或 **Deploy Anyway**（忽略错误部署） |
...
Host 与扩展之间有两条通信通道，自动切换：
1. **Native Messaging**：Chrome 自动启动 Host 时使用（stdin/stdout 管道，低延迟）
2. **HTTP 长轮询**：手动启动 Host 时使用（扩展主动轮询 `GET /api/v1/extension/poll`，发送响应 `POST /api/v1/extension/response`）
...
重启 Chrome 后，在 `chrome://extensions/` 页面刷新扩展，Chrome 会自动启动新代码的 Native Host。

手动启动模式只需 `Ctrl+C` 停掉当前 `node host.js`，然后重新运行即可。
...
```
外部程序 → HTTP API → Node.js Host → Native Messaging / HTTP 长轮询 → Chrome Extension → 目标页面
```

这一架构源于 Chrome MV3 的限制：Service Worker 无法直接运行 HTTP 服务器。Node.js 进程同时承担 HTTP 服务器和 Native Messaging Host 两个角色，避免多进程部署的复杂性。当 Native Messaging 不可用时，自动回退到 HTTP 长轮询模式。

### 多步骤编排引擎

服务由步骤图（Step Graph）定义，每个步骤包含脚本、条件、成功/失败跳转目标和最大迭代次数：

- **有向图执行**：`onSuccess`（成功→下一步）/`onFailure`（失败/放弃→兜底）指向下一个步骤 ID，或 `TERMINATE`（终止）
- **轮询/重试**：`maxIterations>1` 的步骤返回 `{ done:false }` 时自动重跑自身，返回数据或 `{done:true}` 时走 `onSuccess`；预算耗尽或返回 `{failed:true}` 走 `onFailure`（不再使用 `SELF` 哨兵）
- **双迭代保护**：每个步骤的 `maxIterations` + 全局 `maxStepIterations`（默认 50），防止失控循环
- **步骤间数据传递**：`__stepResults__`（按步骤 ID 索引的历史结果）和 `__lastResult__`（上一步结果，跨同一步的重试携带状态）

### AI 驱动的脚本生成与修复

1. **两轮 HTML 协议**：第一轮发送压缩的 DOM 结构摘要（~8000 tokens），LLM 返回候选选择器；第二轮只获取候选元素的完整 HTML 供 LLM 确认或修正。避免截断问题，保持 token 效率。
2. **步骤级自动修复**：执行失败时，只重写失败步骤的脚本，保留步骤图拓扑。修复时使用当前页面的 DOM 快照 + 错误上下文 + 前序步骤结果。
3. **元素标注辅助**：当 LLM 对选择器置信度低于阈值时，自动触发可视化元素标注模式，将用户意图转化为结构化注解。

### 跨 iframe 采集

所有 `$` API 自动穿透同源 iframe。`querySelectorDeep` 搜索主文档和所有同源 iframe 的 DOM；`$list` 聚合所有文档中的匹配元素。这对政府公告网站等大量使用 iframe 嵌套的场景至关重要。

### 安全沙箱

用户脚本在专门的 sandbox iframe 中执行（`manifest.json` 的 `sandbox` 配置），这是 MV3 下唯一允许 `new Function()`/`eval()` 的上下文。脚本只能通过白名单 API（`$`、`$click` 等）与目标页面交互，无法访问 `chrome.*` API 或页面的 JavaScript 上下文。
...
已识别的改进方向（任务取消、Service Worker 保活、存储优化、可视化调试、自适应等待、状态持久化等）已迁移到独立文档：[`docs/roadmap.zh-CN.md`](./docs/roadmap.zh-CN.md)。
