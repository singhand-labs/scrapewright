...
SCROLL-COLLECT RELIABILITY: when the contract promises N items, do NOT hand-roll the scroll+count loop — a transient stall is not exhaustion. Use \$collectUntil(containerSel, {targetCount, idAttr?}) → {satisfied, collected, totalContainers, targetCount, rounds, trace[], exhaustion?}: it scrolls with the full throttle stack (activation, stall detection, trusted-wheel fallback, inner-container probing), counts UNIQUE items (dedupe by idAttr — pass the per-item link attribute so virtualization remounts don't double-count), settles between rounds, and returns a CERTIFIED exhaustion verdict (exhaustion.certified + evidence) only when the scroll machinery itself stalled AND the unique population was unchanged for two consecutive rounds. On exhaustion.certified the shortfall is the page's honest content — ship it disclosed. Example: const r = await \$collectUntil("div[role=feed] div[aria-posinset]", {targetCount: __input__.count, idAttr: "href"}); if (!r.satisfied && !(r.exhaustion && r.exhaustion.certified)) { /* keep scrolling/retry */ }.

HOVER ENRICHMENT (hovercard / link-preview fields): some sites surface richer data — group name, member count, account bio, profile image URL — only in a popover that appears when the user hovers a link. The popover is NOT in the list DOM; it's injected into a portal layer on hover. Use \$hover to fire the popover, then parse fields out of htmlSnippet with \$extractListMulti on a temporary DOM root. Signature: \$hover(anchorSelector, popoverSelector?, opts?) → { hovered: bool, htmlSnippet: string|null, popoverSelector: string|null, reason?: 'popover_timeout'|'hover_failed'|<bg reason> }. A \$hover result may carry rejectedAddedTexts / rejectedAddedHtml — text and HTML fragments READ out of hover-mounted nodes the visual popover filter rejected (hidden, zero-height, or sub-50px mounts; a tooltip strip whose layout never materialized still carries its payload as text). If the value you need appears there, bind the field from it directly — reads are not visibility-gated, so do not keep re-hovering waiting for a visible popover that never renders.
...
// 137th log: `new Function` in an extension PAGE context (wizard) is blocked
// by MV3's default CSP (script-src 'self', no unsafe-eval) — it throws an
// EvalError for EVERY script, valid or not. The 136th update gate shipped
// with exactly that flaw and rejected every service.update in production;
// this gate (the 98th deploy check) has the same latent bug because no real
// deploy was clicked in a page context post-98 — node tests have no CSP.
function isCspConstructError(e) {
  const msg = String((e && e.message) || e);
  return /unsafe-eval|Refused to evaluate a string as JavaScript|Content Security Policy.*(eval|Function)/i.test(msg);
}

// Test seam for the CSP branch: node cannot produce a CSP EvalError from
// `new Function`, so tests inject the failure here.
var __testConstructStepScript = null;
...
function validateForExecution(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    try {
      ...
      if (typeof WU_EXPORT_BAG !== 'undefined' && WU_EXPORT_BAG.__testConstructStepScript) WU_EXPORT_BAG.__testConstructStepScript(step.script);
      else new Function('__input__', '__stepResults__', '__lastResult__', 'return (async function(__input__) { ' + step.script + '\n })(__input__)');
    } catch (e) {
      // 137th log: a CSP-blocked construction environment cannot parse
      // ANYTHING — that is an infrastructure limitation, not a script
      // defect; fail open with a warning (the session update gate's
      // sandbox probe is the primary construction defense).
      if (isCspConstructError(e)) {
        warnings.push(`Step ${step.id}: script parse unavailable in this context (page CSP blocks construction) — the session update gate's sandbox probe is the syntax defense.`);
        continue;
      }
      return {
        valid: false,
...
// 116th round (speed spec track A): parked asks (io.confirm / annotate.request
// / user.observe) once cost a 40-minute wall wait — the title ❓ and the one
// 5-minute toast were invisible to a user in another window. Desktop
// notifications with a bounded re-issue cadence; clicking one focuses the
// wizard tab. Factory-injected so tests drive a fake notifications API.
function createParkNotifier(notificationsApi, focusFn, opts) {
  const o = opts || {};
  const intervalMs = (typeof o.intervalMs === 'number' && o.intervalMs > 0) ? o.intervalMs : 10 * 60 * 1000;
  const maxReminders = (typeof o.maxReminders === 'number' && o.maxReminders >= 0) ? o.maxReminders : 2;
  let timer = null;
  let reminders = 0;
  let seq = 0;
  function fire(kind, question) {
    try {
      let iconUrl = 'icons/icon128.png';
      try { iconUrl = chrome.runtime.getURL('icons/icon128.png'); } catch (e2) { /* tests/headless */ }
  ...
  try {
    if (notificationsApi && notificationsApi.onClicked && typeof notificationsApi.onClicked.addListener === 'function') {
      notificationsApi.onClicked.addListener(function () { try { focusFn(); } catch (e) {} });
    }
  } catch (e) { /* listener registration is best-effort */ }
  return {
    begin: function (kind, question) {
      this.end();
      fire(kind, question);
      reminders = 0;
      const tick = () => {
        if (reminders >= maxReminders) { timer = null; return; }
        reminders += 1;
        fire(kind, question);
...
// 6/6 records) through a green verify because no detector looked at
// cross-record equality of ONE
// field (detectDuplicateEntityPairs compares whole records; this is the
// single-field complement).
function detectIdenticalFieldValues(data, outputSchema) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return [];
  if (!outputSchema || typeof outputSchema !== 'object') return [];
  const props = outputSchema.properties && typeof outputSchema.properties === 'object'
    ? outputSchema.properties : {};
  ...
  for (const key of Object.keys(props)) {
    ...
    for (const fk of fieldKeys) {
      const first = arr[0] && arr[0][fk];
      if (first === null || first === undefined || first === '') continue;
      if (typeof first !== 'string' || first.length < 20) continue; // tiny shared values (flags) are common — only substantial payloads
      let same = true;
      for (let i = 1; i < arr.length; i++) {
        const v = arr[i] && arr[i][fk];
        if (v !== first) { same = false; break; }
      }
      if (same) {
        out.push({
          field: fk, path: key + '.' + fk,
          totalRecords: arr.length,
          sample: String(first).slice(0, 80),
          note: 'identical in every record — a container-level/decoy value (the anti-scrape decoy markup or a shared wrapper), not per-record content: re-bind this field to a per-record element'
        });
      }
    }
  }
