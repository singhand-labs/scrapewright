...
(function (global) {
  ...
  function buildDslContractPrompt() {
    // 138th log (user directive): the accumulated 138 rounds of teaching had
    // grown the base contract to ~21.8K chars of accreted narrative — one
    // undifferentiated dump the model could no longer navigate. Rewritten
    // operative-only: every rule below is a BEHAVIOR the model must follow;
    // the incident histories that justified them live in the repo logs and
    // knowledge units, not here. A guard test pins the size so the base
    // cannot silently regrow (add a rule → cut a stale one, or move depth
    // into a knowledge unit).
    return [
      '# Contract quick index',
      '1 Service model · 2 $ API (2.1 reads / 2.2 lists+hover / 2.3 scroll+collect / 2.4 interaction / 2.5 tabs / 2.6 selector+return rules) · 3 Methodology (10 rules) · 4 Skeleton view',
      '',
      '# 1 Service model',
      'A service is a step graph: steps[{id,name,script,onSuccess,onFailure,maxIterations?}]. onSuccess/onFailure point at another step id or "TERMINATE" — every id exactly once, every target must exist. A step with maxIterations>1 is a poll/retry loop: return {done:false} to retry the same step, {done:true} or plain data to advance via onSuccess, {failed:true} or {error:"..."} to follow onFailure.',
      'Step scripts run as the BODY of an async function. Top-level `await` works directly. Every script MUST contain a top-level `return <value>;` — a script that yields undefined stores no result (verify reports STEP_NO_RETURN). Do NOT wrap the script in `return (...)` or an extra IIFE; statement scripts (`const x = await $(...); return {...};`) are the norm. Await every $ call you depend on before returning. Globals: __input__ (run input), __stepResults__ (prior results by step id), __lastResult__.',
      '',
      '# 2 $ API',
      '## 2.1 Reads',
      '- $(sel) — first matching element data. $count(sel) → number. $list(sel) → plain-JSON element data (NOT live DOM nodes — you cannot call querySelectorAll on them).',
      '- $exists(sel, timeoutMs?) → boolean — VISIBILITY-gated (false for display:none/zero-size); reads are NOT gated, so never guard a read with it.',
      '- $extract(sel, attr?) → string — BARE return, assign directly (postTime = await $extract(sel)); attr may be "outerHTML"/"innerHTML" or any attribute name; no attr = trimmed textContent. Assign directly: Element-HTML reads are capped at 50000 chars with a `<!--TRUNCATED: ...-->` suffix disclosing the original length.',
      '- $labelledby(sel, attr?, timeoutMs?) → {text, attr, refCount, missingIds?, note?, viaDescendant?} — resolves the ARIA reference attr (default aria-labelledby; "aria-describedby" opt-in) and returns the CONCATENATED text of the referenced elements — values that live only in hidden-but-readable tooltip spans. When the match carries neither reference attribute, resolution DESCENDS to its first descendant that carries one (viaDescendant + note disclose it). postTime = (await $labelledby(sel)).text — tooltip/timestamp full values usually live here; try it BEFORE re-hovering. probe.labelledby returns the same shape. Environmental claims need evidence too: before concluding "requires login"/"unauthenticated", call probe.loginState and quote its marker census — page shape alone is not login-state evidence.',
      '- $timestamp(containerSel, {anchorSel?, timeoutMs?, index?}) — the ONE-CALL timestamp dance for a card (labelledby + aria-label + visible text + hovered popover text, date-shape filtered, absolute preferred over relative). postTime = (await $timestamp(cardSel)).value; judge candidates[] yourself when the heuristic misses. The research-side twin probe.timestamp (session tool) does the same dance in one call — THE first move whenever postTime comes back relative or junk.',
      '',
      '## 2.2 Lists + hover',
      '- $extractList(containerSel, fieldMap, opts?) — one record per container. fieldMap {field:{selector,attr?,labelledby?,multi?}}: multi:true collects ALL matches of that field per container as an ARRAY (document order, [] when none — the exact envelope probe.extract returns with multi:true; a NON-multi field is a scalar string, and for-of over a string iterates characters); labelledby:true (or the attr name) resolves the ARIA reference on the match, WITH the same descendant fallback as $labelledby — the read for anti-scrambled textContent (visible text is decoy junk; the clean value lives in the referenced hidden elements). opts.allowEmpty waives the empty-throw and returns every record (never filter to []). Anchor outerHTML/innerHTML fields to the SEMANTIC sub-element, not the whole card (whole-card DOM is class/style/SVG noise; verify flags such fields OUTPUT_FIELD_SIZE); HTML values cap at 50000 chars with a TRUNCATED suffix.',
      '- $extractListMulti(containerSel, fieldMap, opts?) — every field array-valued; equals all specs carrying multi:true. Use for whole-call multi; use per-field multi:true when only some fields need all matches.',
      '- $extractWithHover(containerSel, fieldMap, {hover:{anchorSel, popoverSel?}}) — same fieldMap shape as $extractList (multi:true included). anchorSel is evaluated INSIDE each container; a comma-UNION hovers several anchors per card. Each record carries `hovercards[]`, ONE ENTRY PER matched anchor: {hovered, htmlSnippet, popoverSelector, reason, observedPopover, anchorIndex, anchorHref, anchorText, labelledbyText, labelledbyAttr} (probe.hover shows the same envelope for one anchor). Build hover-card output fields from r.hovercards: htmlSnippet is the captured popover DOM; anchorHref resolves the nearest ENCLOSING a[href] (pointing anchorSel at an inner span still classifies by the link it sits in); labelledbyText is the anchor\'s accessible label harvested at dwell time (same descendant fallback; present on FAILED entries too — a label needs no visible popover, so a label-anchor entry is exactly where to take a field\'s value from, NOT something to filter out). There is NO popoverHtml or __popover field. COST: every anchorSel union member costs a FULL hover cycle (~5-10s) per card — narrow the anchorSel union to the ONE anchor the hover-derived fields need (one anchor per card is usually enough). For a tooltip-valued field, point anchorSel at that element and bind the value via read:\'hoverPopover\' with your own match regex (the channel searches the picked popover PLUS rejectedAddedHtml/rejectedAddedTexts — values the visual filter rejected remain reachable).',
      '- $hover(anchorSel, popoverSel?, opts?) — one-off trusted hover (opts.index picks the Nth anchor); for record extraction prefer $extractWithHover.',
      '',
      '## 2.3 Scroll + collect',
      '- $scrollBy(px), $scrollToBottom(), $scrollIntoView(sel) — all scroll ops auto-activate the tab AND re-focus its window (the user may switch away mid-run; every call switches back). A no-progress $scrollBy carries pageState (real visibilityState/hasFocus) and, when gated, frameSample (rAF ticks over ~300ms): ~0 ticks = the renderer stopped producing frames (lazy-load cannot fire; activation is automatic, `scrapewright throttle on` covers occluded windows); normal ticks with a stable count = genuine exhaustion. When the window cannot move, $scrollBy falls back to the inner scrollable container (fallback:"inner-container").',
      '- $collectUntil(containerSel, {targetCount: N, idAttr}) is the DEFAULT for a count-bounded feed: settles between rounds, counts UNIQUE items (virtualization remounts do not double-count), probes the inner scroll root, and returns a CERTIFIED exhaustion verdict. Hand-roll a poll step only when $collectUntil cannot express the loop — then: maxIterations>1, $scrollBy one viewport per iteration, an AWAITED SETTLE in the not-ready branch, $count the population, return {done:false} while count < N and {done:true} with the data at N. Reserve $scrollToBottom for genuinely collect-everything requirements.',
      '',
      '## 2.4 Interaction + waits',
      '- $click(sel), $type(sel, text), $check(sel, prop), $clickInList(containerSel, subSel).',
      '- $wait(sel, ms?) — waits for a selector (30s cap) then sleeps ms; THROWS ELEMENT_NOT_FOUND if it never appears. Never $wait on a selector whose ABSENCE is your poll condition — $count it and return {done:false} so maxIterations drives the wait.',
      '- $waitForStable(sel, opts?) — resolves true once sampled content stops changing (streaming content); prefer over setTimeout guessing. A false resolve\'s diagnostics carry pageState — hidden/unfocus means the tab was throttled (content may never have arrived), not that the text kept changing.',
      '',
      '## 2.5 Tabs',
      '- $openTab(url, fn) — open a sub-tab, run fn in it, return its result.',
      '',
      '## 2.6 Selector + return rules',
      'Every $ call resolves to plain serializable JSON (or a primitive) — live DOM nodes and un-awaited Promises cannot cross the sandbox boundary.',
      'The $ list above is exhaustive — there is no $json, $log, $fetch, or any other $ global; use the plain JS builtins (JSON.stringify, Math, Array methods).',
      'Selectors are STANDARD CSS only (:has/:not/:is/:where are valid; :has-text/:contains/:text= are NOT and throw instantly).',
      'Selectors may target iframes with the prefix <iframe-css>|<inner-css> — works in every $ API.',
      '',
      '# 3 Methodology (10 rules)',
      ...
      '3. Attribute DISTRIBUTIONS (probe.attrStats), not single samples, reveal what a data-* attribute means (attr is read on the elements containerSel matches, not their descendants). :has()/:not(:has()) filter by DESCENDANTS — before shipping a clause on an attr, census the descendant form containerSel + " [attr]": absentPct 100 on the element form does NOT mean the attr is absent inside your containers.',
      ...
      '6. To observe a hover popover during research call probe.hover (the session tool — NOT the $hover DSL primitive). When it observes a popover, copy the returned canonical popoverSelector VERBATIM into popoverSel — an embellished variant is a new string the gate rejects. A no-popover result may carry rejectedAddedTexts/rejectedAddedHtml — text/HTML read out of hover-mounted nodes the visual filter rejected: the popover "exists" as readable content even when it never rendered visually; bind the field from that evidence directly. Try probe.labelledby on the anchor FIRST (tooltip/timestamp values very often live in the referenced hidden span). Popover absence is ANCHOR-specific evidence — hover at least one other anchor (on a repeating item, the element that carries identity or author metadata is the usual hovercard carrier) before concluding popovers do not work here; when two different anchors show no popover, stop. CHOOSE THE ANCHOR BY REQUIREMENT SEMANTICS, not element type: any element can carry a hover handler (span, abbr, time, div, li — not just links); to read a timestamp tooltip, hover the timestamp element itself.',
      '7. Research scrolling: use probe.scroll / probe.scrollUntil (session tools — NOT the $scroll primitives). REQUIREMENT-BOUNDED: when the requirement names a count (N items), the scroll loop is BOUNDED by that count — call probe.scrollUntil({sel, targetCount:N}) — it stops the MOMENT the population reaches N; never scroll to exhaustion for a bounded requirement, never hand-cycle scroll→count. READ THE TRACE: a count that NEVER changes while page height grows means sel matches static page chrome instead of the growing population — a wrong selector, not missing data (re-target sel before scrolling again); count AND height both frozen = the SCROLL ROOT TESTED is exhausted (an at_bottom note / fallback:"inner-container" says when to retry with scrollSel at the feed\'s own scrollable container); count frozen at a NONZERO value with no scroll progress is ambiguous between renderer gating and exhaustion — read pageState/frameSample (hidden/unfocused + ~0 rAF ticks = throttled; normal frames = exhausted: accept the count or renegotiate via io.confirm) instead of rewriting the scroll step. A container selector you scroll or count becomes an observation receipt grounding later step-side scroll calls.',
      '8. Iterate the fieldMap in the LIVE tab with probe.extract BEFORE writing it into service.update (one probe turn per revision, empty-field census included); reserve service.update + verify.run for the end-to-end check. Verify opens a FRESH tab — cold-load divergence (fewer/different items than the research tab) is expected; investigate counts with research-tab probes, not by re-verifying. Run the FIRST verify with the SAME input values that drove the research page (a different input value can change the result-card population ENTIRELY — population divergence, not a rendering failure; FIELD_MATCH_ZERO names the census). After a green verify, spot-check one other input. INPUT_VALUE_SUSPECT (zero containers at all): the input VALUE is the prime suspect — the site may simply have no content for that value (not a selector bug) — re-run verify.run with {"input":{<param>:<a DIFFERENT, more common value>}} BEFORE hardening selectors; if it succeeds, adopt via service.update({testInput:{...}}) (steps-less) and re-verify without an override. BUT read the differential first: SELECTOR_OVERFILTERED (or [differential: ...] showing the stripped base matched >0) means your own trailing :not()/:has() clause removed the population — census each clause and fix the clause, not the input value.',
      '9. EARLY contract confirmation: after the first page.open and a coarse look at the repeating item, propose io.confirm({inputSchema, outputSchema, testInput, note}) and WAIT — service.update is rejected until the user confirms and annotate.request is likewise rejected — settle the contract first. Ensure the page is RENDERED, not just loaded (ready/bodyTextChars say loaded; a shell-sized body on a JS-heavy site is still hydrating — page.settle first; probing an unhydrated shell yields timing evidence, not page evidence). Apply every user revision and re-confirm. A re-proposal with unchanged shape AND test values auto-confirms without prompting; propose again only on user request or a MATERIAL change (fields added/renamed/removed, type changes, new test values — description-only edits are exempt): call io.confirm again with the new schemas first. PROPOSE testInput TOGETHER WITH the schemas ({keyword:"...",count:N} — the values every verify.run sends; propose the values you actually researched with). After confirmation, service.update may carry steps only — the confirmed schemas attach to the artifact automatically; a schema-only service.update also lands the contract on an existing artifact. Changing test values later requires re-confirmation (TEST_INPUT_UNCONFIRMED otherwise).',
      '10. Ship real values only. Junk shapes a green verify can still carry: bare query strings posing as ids, data: URIs in url/media arrays (inline UI icons — filter arrays to http(s) entries inside the step script), raw HTML dumps in data fields, OBFUSCATED text (anti-scrape decoys: interleaved/scrambled runs, reversed fragments, zero-width/combining chars). If a value reads scrambled, the clean value usually lives in an ATTRIBUTE on the same element (aria-label, title, datetime) — probe it and bind the field to that attribute; never ship an obfuscated "best-effort" value. If research proves a confirmed field is unextractable or only junk-reachable, renegotiate the contract with io.confirm (drop or redefine the field) instead of shipping it empty/junk. Scalar or single-value outputs skip the array filters but still go through detectors.junkValues. Check detectors.junkValues and detectors.partialEmptyFields in the verify report. An empty string is not a value either: a confirmed field empty in EVERY record (emptyRatio 1) is a binding failure or the page lacks the data — fix the binding (attribute fallback) or renegotiate with io.confirm; empty in only SOME records may legitimately vary — if so and the field is required, move it to optional in the contract. Never rationalize a persistent empty as timing, never hardcode an empty-string placeholder, and never declare synthetic bookkeeping fields (index, serialNumber, loop counter) — a populated synthetic field masks the empty-data signals.',
      '',
      '# 4 Skeleton view & model-directed cleaning',
      'HTML evidence rides as a NUMBERED SKELETON view ([n1], [n1.2] hierarchical node numbers; noise tags stripped, anonymous div/span wrappers collapsed, semantic attributes + visible text kept; hidden-but-readable aria carrier spans KEPT — they hold labelledby values). probe.sample/probe.snippet return HTML fields as skeletons by default (opts.raw:true keeps the original). probe.skeleton({sel, index?, opts?}) gives any selector\'s numbered skeleton with CLEANING KNOBS YOU DIRECT: opts.stripTags (default script/style/noscript/template/svg/path/link/meta/iframe), opts.keepAttrs (default id, first-3 classes, role, aria-*, href, data-*, title, alt, type, name, value), opts.maxTextLen (200), opts.maxDepth (30), opts.capChars (8000/container, 20000 full). Cite nodes by number ([n3.2]) in findings and regexes. The EVIDENCE DOSSIER block (rebuilt every turn) carries the representative container skeleton, captured popover texts, the last verify census, and the current artifact\'s FULL step scripts — write regexes and bindings against the dossier skeleton, never an imagined shape.'
    ].join('\n');
  }
  ...
  function createSessionTools(deps) {
    ...
    async function ioConfirm(args, ctx) {
      ...
      try {
        ...
        // 139th log: the confirmed contract declared the synthetic bookkeeping
        // field `index` as REQUIRED — rule 10 forbids declaring it at all (a
        // populated synthetic field masks empty-data signals), and prompt-only
        // teaching has not stopped models from re-offering it. Warn in the
        // panel (the user stays the authority on the contract — warning, not
        // rejection) whenever an array's items.required carries a
        // bookkeeping-named field.
        try {
          const BOOKKEEPING_RE = /^(index|idx|serialnumber|serial_no|rownumber|rownum|seq|sequence|序号|行号)$/i;
          const props = (a.outputSchema && a.outputSchema.properties) || {};
          for (const k of Object.keys(props)) {
            const items = props[k] && props[k].items;
            if (!items || !Array.isArray(items.required)) continue;
            const hits = items.required.filter((f) => BOOKKEEPING_RE.test(String(f)));
            if (hits.length) {
              diffLines.push('SYNTHETIC REQUIRED FIELD — "' + k + '" requires bookkeeping field(s) [' + hits.join(', ') +
                ']: an index/serial number is generated by the extraction loop, not data the page carries — it masks empty-data signals (a record whose only filled field is index reads as non-empty). Confirm only if you specifically want it.');
            }
          }
        } catch (e) { /* the warning is best-effort, never blocks confirmation */ }
    ...
    async function serviceUpdateGated(args, ctx) {
      ...
      // 138th log: set by the assembly dedupe below; stamped on the receipt.
      let chunkDedupeNote = null;
      try {
        const a0 = args || {};
          ...
          if (Array.isArray(a0.steps) && a0.steps.length) {
            ...
            return {
              ...
              note: 'pending update chunks cleared; the ' + a0.steps.length +
                ' step(s) sent with this call were buffered as the NEW chunk 1 — send ONLY the REMAINING steps as the final chunk (service.update({steps:[...]}) assembles). These ids are already buffered: [' +
                a0.steps.map((s) => String((s && s.id) || '(no id)')).join(', ') +
                '] — resending one supersedes the buffered copy (last definition wins), but a clean final chunk carries only the steps not sent yet.'
            };
          }
        ...
        // abortChunk carrying the FULL graph buffered 4 steps, the model
        // then resent all 4 as the "final chunk", and the concat assembled
        // 8 steps with 4 duplicate ids — validateChain's id-keyed checks
        // passed it and v8/v9 shipped a shadowed graph. Same-id resends are
        // a legitimate model shape ("here is the complete artifact"); the
        // LAST definition of each id wins (the final chunk is the fresher
        // copy), duplicates are dropped, and the receipt discloses it.
        if (Array.isArray(a0.steps) && a0.steps.length > 1) {
          const lastIdx = new Map();
          for (let di = 0; di < a0.steps.length; di++) {
            const ds = a0.steps[di];
            if (ds && ds.id != null) lastIdx.set(String(ds.id), di);
          }
          if (lastIdx.size < a0.steps.length) {
            const keptSteps = [];
            const droppedIds = [];
            for (let di = 0; di < a0.steps.length; di++) {
              const ds = a0.steps[di];
              const dkey = (ds && ds.id != null) ? String(ds.id) : null;
              if (dkey != null && lastIdx.get(dkey) !== di) { droppedIds.push(dkey); continue; }
              keptSteps.push(ds);
            }
            a0.steps.length = 0;
            for (const st of keptSteps) a0.steps.push(st);
            chunkDedupeNote = 'chunk assembly DEDUPED ' + droppedIds.length +
              ' duplicate-id step definition(s) [' + Array.from(new Set(droppedIds)).join(', ') +
              '] — the LAST definition of each id wins. Steps applied: [' +
              keptSteps.map((s) => String((s && s.id) || '(no id)')).join(', ') + ']';
          }
        }
      ...
      try {
        ...
        if (lastVerify) lastVerify.staleArtifact = true;
        if (attached || attachedTestInput || parseCheckNote || chunkDedupeNote) {
          ...
          const version = (st && st.session && Array.isArray(st.session.artifactVersions))
            ...
            : 1;
          const parts = [];
          if (chunkDedupeNote) parts.push(chunkDedupeNote);
      ...
      const version = (st && st.session && Array.isArray(st.session.artifactVersions))
        ? st.session.artifactVersions.length + 1
        : 1;
      const receiptNotes = [chunkDedupeNote, parseCheckNote].filter(Boolean).join('; ');
      return Object.assign({ updated: true, version: version }, receiptNotes ? { note: receiptNotes } : {}, staticLint.length ? { staticLint: staticLint } : {}, endgameWarning(ctx));
    }
    ...
    const toolSpecs = [
      // 138th log: operative-only specs — args, return shape, and the call-time
      // rules. Deep teaching lives in the knowledge units and the contract
      // base; a guard test pins the total size.
      { name: 'page.open', args: '{url?}', returns: '{tabId,url,ready,bodyTextChars?,warning?} — {{param}} placeholders fill from the test input; a URL that still has one is rejected. bodyTextChars censuses rendered text: ready:true only means loaded — a tiny body is an unhydrated shell (page.settle next)' },
      { name: 'page.state', args: '{}', returns: '{open,tabId,url,title,status}' },
      { name: 'page.settle', args: '{sel?,timeoutMs?}', returns: '{settled,sel,chars,waitedMs,textHead?,note?} — polls until content stops changing (default sel body). Settle before concluding a JS-heavy page is empty' },
      ...
      { name: 'probe.loginState', args: '{}', returns: '{loggedIn, loginWall, evidence, note} — login/logout marker census. Quote it BEFORE any "requires login"/"unauthenticated" claim; page shape alone is NOT login-state evidence' },
      { name: 'probe.timestamp', args: '{containerSel, anchorSel?, index?}', returns: '{heuristicValue, value, absolute, relative, candidates[{value,source,relative}], note?} — ONE call does the whole timestamp dance on one card (hovers time-ish anchors, harvests labelledby/aria/text, date-shape filtered, absolute preferred). Judge candidates[] yourself when the heuristic misses. FIRST move whenever postTime is relative or junk' },
      { name: 'diag.read', args: '{stepId?, kind?}', returns: '{selectorDiagnostics, failingStep?, popover, counters, lastError?} — kind:"contract" (no verify needed) shows confirmation/schema state BEFORE verify.run; kind:"unusedCaptures" returns the verify report\'s unconsumed popover captures' },
      { name: 'verify.run', args: '{input?, preflight?} — optional input override for THIS run (the INPUT_VALUE_SUSPECT alternate-value re-test); preflight:true dry-runs the extract step on the warm research tab first (default off)', returns: '{ok,score,error,detectors,steps,resultDebug?,finalResult,schemaOk} — detectors.partialEmptyFields: confirmed fields that came back empty, each with emptyRatio and emptyRecordSamples (WHICH records: 1-based ordinals + content hints) — probe THOSE record shapes on the research tab instead of blind-rewriting. detectors.emptyFieldDiagnostics: per-field falsification crumbs (missingIds, 0-container sub-selectors, absent attrs) — read BEFORE re-probing. resultDebug surfaces your step\'s small non-record debug keys. A field populated on the research tab but empty in verify depends on page state the warm tab ACCUMULATED — re-derive on a fresh page' },
      ...
      { name: 'user.observe', args: '{question, hint?}', returns: '{answer} | {cancelled} — ask the USER what they observe when the harness cannot perceive it (popovers that never visibly mount, login/geo/count variance). Their eyes are the best sensor available: one user.observe beats rounds of blind re-probing; the answer lands in the ledger; the wait does not consume the session clock' },
      { name: 'probe.snippet', args: '{code, timeoutMs?}', returns: '{result, truncated?} — run an ARBITRARY $-DSL snippet (async body, top-level await+return) on the research tab, RAW result back. Test-before-artifact: verify extraction/assembly logic here BEFORE writing it into service.update. timeoutMs default 30000 max 90000 — each hovered anchor burns ~5-10s, so size the batch (maxContainers/containerRange) or the budget; resending the identical oversized snippet cannot change the outcome. HTML-bearing fields return as NUMBERED SKELETONS by default (raw:true keeps originals)' },
      { name: 'probe.skeleton', args: '{sel, index?, opts?, raw?}', returns: '{total, match, skeleton} — numbered skeleton view of the Nth match; opts.{stripTags[],keepAttrs[],maxTextLen,maxDepth,capChars} direct the cleaning. Write regexes and bindings against THIS view (and the dossier container skeleton), never an imagined shape' },
      { name: 'io.confirm', args: '{inputSchema, outputSchema, testInput?, note?}', returns: '{confirmed:true} | {confirmed:false, feedback} — propose EARLY and WAIT; service.update is rejected until confirmed; a same-shape (and same-test-values) re-proposal auto-confirms without prompting. testInput carries the CONCRETE test request values — propose the ones you researched with; the user confirms/edits them in the same panel. outputSchema MUST declare fields (properties + required); a fieldless {"type":"object"} verifies blind and is rejected' }
    ];
