...
(function (global) {
  ...
  function buildDslContractPrompt() {
    return [
      ...
      '- $labelledby(sel, attr?, timeoutMs?) — resolves the ARIA reference attr (default aria-labelledby; pass "aria-describedby") on the first match and returns the CONCATENATED text of the referenced element(s) (id list looked up by id). Tooltip/timestamp full values usually live there — the referenced span is often hidden but readable, so use this instead of re-hovering when the popover never visibly renders',
